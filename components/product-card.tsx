    ...
    >
      <Card className="overflow-hidden bg-white transition-shadow duration-300 group-hover:shadow-lg">
        <Link href={`/productos/${product.slug}`}>
          <div className="relative aspect-square overflow-hidden">
            ...
            {product.inStock && (
              ...
              >
                <Button
                  size="sm"
                  className="bg-[var(--brand-500)] hover:bg-[var(--brand-600)] text-white shadow-lg"
          ...
          <div className="p-4">
            <h3 className="mb-2 text-lg font-semibold text-[var(--fg)] group-hover:text-[var(--brand-600)] transition-colors">
              {product.name}
            </h3>
            ...
            <div className="mt-3 flex items-center space-x-2">
              <span className="text-xs text-[var(--muted)]">Colores:</span>
              <div className="flex space-x-1">
                {product.colors.slice(0, 3).map((color, index) => (
                  <div
                    key={index}
                    className="h-4 w-4 rounded-full border border-[var(--brand-200)]"
                    style={{
