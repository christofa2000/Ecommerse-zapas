...
test.describe('Products Page', () => {
  test('should load products page and display all products', async ({ page }) => {
    await page.goto('/productos');
    
    // Check page title
    await expect(page.getByText('Todas las Zapatillas')).toBeVisible();
    await expect(page.getByText('Explora nuestra colección completa de zapatillas sostenibles y cómodas.')).toBeVisible();
  });
  ...
  test('should show empty state when no products match filter', async ({ page }) => {
    ...
    await expect(page.getByText('Intenta ajustar los filtros o explorar otras categorías.')).toBeVisible();
  });
